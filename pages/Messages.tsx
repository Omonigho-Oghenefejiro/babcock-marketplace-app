...
const Messages = () => {
  ...
  return (
    <div style={{ background: t.cream, height: 'calc(100vh - 4rem)', display: 'flex', flexDirection: 'column', fontFamily: "'Instrument Sans', sans-serif" }}>
      <div style={{ background: t.green, padding: 'clamp(10px, 3vw, 14px) clamp(12px, 5vw, 24px)', display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
        ...
        <div style={{ minWidth: 0 }}>
          <h1 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: 'clamp(0.85rem, 2vw, 1rem)', color: '#fff', lineHeight: 1 }}>Messages</h1>
          <p style={{ fontSize: 'clamp(0.65rem, 1.5vw, 0.72rem)', color: 'rgba(255,255,255,0.5)', marginTop: 2 }}>{myConvs.length} conversation{myConvs.length !== 1 ? 's' : ''}</p>
        </div>
      </div>
          ...
          <div style={{ padding: '14px 16px', borderBottom: `1px solid ${t.border}` }}>
            <div style={{ position: 'relative' }}>
              ...
              <input
                ...
                style={{ width: '100%', paddingLeft: 30, paddingRight: 12, paddingTop: 9, paddingBottom: 9, background: t.cream, border: `1.5px solid ${t.border}`, borderRadius: 10, fontFamily: "'Instrument Sans', sans-serif", fontSize: 'clamp(0.75rem, 2vw, 0.82rem)', color: t.ink, outline: 'none' }}
              />
            </div>
          </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <p style={{ fontSize: 'clamp(0.75rem, 2vw, 0.78rem)', color: unread > 0 ? t.ink : t.muted, fontWeight: unread > 0 ? 600 : 400, overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>{lastMsg ? (lastMsg.senderId === user.id ? 'You: ' : '') + lastMsg.content : 'Start chatting…'}</p>
                    </div>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, background: t.cream }} className={`messages-chat ${!selectedConvId ? 'hidden sm:flex' : 'flex'}`}>
          {activeChat && activeChatOther ? (
            <>
              ...
              <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', padding: 'clamp(12px, 3vw, 20px) clamp(12px, 4vw, 24px)', display: 'flex', flexDirection: 'column', gap: 'clamp(8px, 2vw, 12px)' }}>
                  ...
                  {(activeChat.messages ?? []).map((msg: any) => {
                    ...
                    return (
                      <motion.div key={msg.id} initial={{ opacity: 0, y: 8, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.2 }} style={{ display: 'flex', justifyContent: isMe ? 'flex-end' : 'flex-start', gap: 8, alignItems: 'flex-end' }}>
                        ...
                        <div style={{ maxWidth: 'clamp(200px, 85%, 72%)' }}>
                          {!isMe && <p style={{ fontSize: 'clamp(0.65rem, 1.5vw, 0.7rem)', color: t.greenMid, fontWeight: 600, marginBottom: 4 }}>{senderName}</p>}
                          <div style={{ background: isMe ? t.green : '#fff', color: isMe ? '#fff' : t.ink, border: isMe ? 'none' : `1.5px solid ${t.border}`, borderRadius: isMe ? '18px 18px 4px 18px' : '18px 18px 18px 4px', padding: 'clamp(8px, 2vw, 10px) clamp(10px, 3vw, 14px)' }}>
                            <p style={{ fontSize: 'clamp(0.8rem, 2vw, 0.875rem)', lineHeight: 1.55, wordWrap: 'break-word' }}>{msg.content}</p>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 4, marginTop: 4 }}>
                              <span style={{ fontSize: 'clamp(0.6rem, 1.5vw, 0.65rem)', color: isMe ? 'rgba(255,255,255,0.5)' : t.muted }}>{new Date(msg.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
              ...
              <div style={{ background: '#fff', borderTop: `1.5px solid ${t.border}`, padding: 'clamp(10px, 2vw, 14px) clamp(12px, 4vw, 20px)', flexShrink: 0 }}>
                <form onSubmit={handleSend} style={{ display: 'flex', gap: 'clamp(6px, 2vw, 10px)', alignItems: 'center' }}>
                  <input
                    type="text"
                    value={inputText}
                    onChange={e => setInputText(e.target.value)}
                    placeholder="Type a message..."
                    style={{ flex: 1, padding: 'clamp(10px, 2vw, 12px) clamp(12px, 3vw, 16px)', background: t.cream, border: `1.5px solid ${t.border}`, borderRadius: 12, fontFamily: "'Instrument Sans', sans-serif", fontSize: 'clamp(0.8rem, 2vw, 0.875rem)', color: t.ink, outline: 'none' }}
                  />
                  <button type="submit" disabled={!inputText.trim()} style={{ width: 'clamp(40px, 10vw, 44px)', height: 'clamp(40px, 10vw, 44px)', borderRadius: 12, border: 'none', background: inputText.trim() ? t.green : '#E5E7EB', color: inputText.trim() ? '#fff' : t.muted, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: inputText.trim() ? 'pointer' : 'not-allowed', transition: 'all 0.2s', flexShrink: 0 }}>
                    <Send size={16} />
                  </button>
                </form>
              </div>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 'clamp(8px, 2vw, 12px)', padding: 'clamp(20px, 5vw, 32px)' }}>
              <div style={{ width: 'clamp(60px, 15vw, 72px)', height: 'clamp(60px, 15vw, 72px)', borderRadius: 20, background: t.greenLight, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <MessageCircle size={30} color={t.green} />
              </div>
              <p style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 'clamp(0.85rem, 2vw, 1rem)', color: t.ink, textAlign: 'center' }}>Select a conversation</p>
            </div>
          )}
        </div>
      </div>
